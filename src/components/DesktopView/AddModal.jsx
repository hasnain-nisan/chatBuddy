        ...
        <Box sx={style}>
          ...
          <div className="flex flex-col gap-10">
            ...
            <div className="flex flex-col gap-6 justify-center items-center">
              <input onChange={(e) => filePreview(e.target)} type="file" name="" id="imageInput"  className="hidden"/>
              <div onClick={uploadFile} className={`group flex flex-col gap-2 items-center justify-center w-36 h-36 bg-transparent rounded-full border-2 hover:border-teal-500 cursor-pointer ${!image ? "border-teal-700" : "border-transparent"}`}>
                {image ? (
                  <img src="" alt="" srcset="" id="group-pic" className="rounded-full object-cover w-36 h-36"/>
                ) : (
                  <>
                    <button className="group-hover:text-teal-500 text-teal-700">
                      <FiUploadCloud fontSize={30} className=""/>
                    </button>
                    <p className="text-teal-700 group-hover:text-teal-500 font-popins text-sm">Group Image</p>
                  </>
                )}
              </div>
              <div class="w-full">
              ...
              <div class="w-full">
                <Select
                  ...
                  className=""
                  placeholder="Select group participents"
              ...
              <div className="w-full">
                <div class="relative inline-block w-8 mr-2 align-middle select-none transition duration-200 ease-in">
                    <input onChange={(e) => setIsPrivateGroup(e.target.checked)} type="checkbox" name="toggle" id="toggle" class="toggle-checkbox absolute block w-5 h-5 rounded-full bg-[#1E1C26] border-4 appearance-none cursor-pointer"/>
                    <label for="toggle" class="toggle-label block overflow-hidden h-5 rounded-full bg-teal-700 cursor-pointer"></label>
                </div>
                <label for="toggle" class={`text-xs font-popins ${isPrivateGroup ? "text-teal-500" : "text-teal-700"}`}>Set group to private</label>
              </div>
              <div className="w-full">
